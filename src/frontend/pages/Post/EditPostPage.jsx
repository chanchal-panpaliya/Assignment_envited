...
const EditPostPage=()=>{
   ...
   const { getEditPostData } = useSelector((store) => store.Menu);
   ...
   //
   const [NameListArray,setNameListArray] = useState(getEditPostData.data.tagName)
   const [url,seturl] = useState(getEditPostData.data.url)
   const [Postcontent,setPostContent] = useState(getEditPostData.data.content)
   const [title,settitle] = useState(getEditPostData.data.title)
   const [gethashtag,sethashtag] = useState(getEditPostData.data.gethashtag)
   const [postType,setType]=useState(getEditPostData.data.postType)
   const [postPolicy,setPolicy]=useState(getEditPostData.data.postPolicy)
   const [postTopic,SetpostTopic]=useState(getEditPostData.data.postTopic)
   const [eventLocation,setEventLocation]=useState(getEditPostData.data.eventLocation);
   const [eventDate,setDate]=useState(getEditPostData.data.eventDate);
    //image loading..
       ...
       if(postType==="EVENT"){
           if(title!=="" && Postcontent!=="" && eventLocation!=="" && eventDate!=="" && postTopic!==""){
               let postData = {
                 ...
                 getEditPostData.page==="Archive"? 
                    dispatch(EditArchivePost({postData,token,toastdispatch})).then(res=>{
                        navigator(`/${user._id}/archive`)
                    }) : 
                    dispatch(editUserPost({postData,token,dispatch,toastdispatch})).then(res=>{
     ...
     <div className='singlepost-container'>
         ...
         <div className='post-button-container'>
             ...
             <button className='post-button' onClick={()=>UploadPost()} disabled={isuploading}> 
               {isuploading?"Image Uploading...":"Update Post"}  
             </button>
         </div>
