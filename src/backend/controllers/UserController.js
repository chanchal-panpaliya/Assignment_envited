...
export const editUserHandler = function (schema, request) { //stories update
  let user = requiresAuth.call(this, request);
  let post = schema.posts.where({ userId: user._id  })?.models;
  try {
    if (!user) {
      return new Response(
        404,
    ...
    post = { ...post.attrs, ...postData };
    this.db.posts.update({ userId: user._id }, post);
